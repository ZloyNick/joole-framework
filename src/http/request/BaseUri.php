...
class BaseUri
{
    ...
    public static function generateQueryString(array $params = []): string
    {
        $queryString = '';
        $lastParam = array_key_last($params);

        foreach ($params as $paramName => $value) {
            $queryString .= is_array($value) ? self::generateQueryParamRecursive($paramName, $value)
                : $paramName . '=' . $value;

            if ($lastParam !== $paramName) {
                $queryString .= '&';
            }
        }
    ...
    private static function generateQueryParamRecursive(string $paramName, array $values): string
    {
        $result = '';
        ...
        foreach ($values as $name => $value) {
            if (is_array($value)) {
                $result .= self::generateQueryParamRecursive($paramName . '[' . $name . ']', $value);

                continue;
            }

            $result .= $paramName . '[' . $name . ']=' . $value;
